...
class UpdaterSchedulingTest : BaseContextTest() {
    ...
    fun `minimum interval skips checks after a successful update, across relaunches`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(noContent(), noContent())

        LingoHub.update()
        relaunch()
        now += 15 * minute - 1
        LingoHub.update()
        verify(api, times(1)).getBundleInfo(any())
    ...
    fun `installed release and nothing published start the minimum interval`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(release(), problem(404, "DISTRIBUTION_NOT_FOUND"))
    ...
    fun `transport error is not retried and the next call checks again`() = runTest {
        whenever(api.getBundleInfo(any())).thenAnswer { throw IOException("offline") }.thenReturn(noContent())
    ...
    fun `5xx is retried once after two to five seconds`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(problem(503), noContent())
    ...
    fun `5xx retry follows Retry-After`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(problem(503, retryAfter = "4"), noContent())
    ...
    fun `persistent 5xx pauses checks with backoff across relaunches`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(problem(503), problem(503), problem(502), problem(502), noContent())
        ...
        now += 1
        LingoHub.update()
        verify(api, times(5)).getBundleInfo(any())
    ...
    fun `Retry-After over ten seconds skips the retry and pauses checks`() = runTest {
        val start = now
        whenever(api.getBundleInfo(any())).thenReturn(problem(503, retryAfter = "1800"))
    ...
    fun `429 pauses checks for an hour across relaunches`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(problem(429, "USAGE_LIMIT_EXCEEDED"), noContent())
    ...
    fun `429 pause follows a longer Retry-After`() = runTest {
        val start = now
        whenever(api.getBundleInfo(any())).thenReturn(problem(429, "USAGE_LIMIT_EXCEEDED", retryAfter = "10800"))
    ...
    fun `client errors are neither retried nor paused`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(problem(401, "CDN_KEY_NOT_FOUND"), problem(401, "CDN_KEY_NOT_FOUND"))
    ...
    fun `failed download gets one fresh check for a new URL`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(release("$FILES_URL?expired"), release("$FILES_URL?fresh"))
    ...
    fun `download that fails again gives up until the next call`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(release(), release(), noContent())
    ...
    fun `fresh check after a failed download is not retried`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(release(), problem(503))
    ...
    fun `transport error during download is not retried`() = runTest {
        whenever(api.getBundleInfo(any())).thenReturn(release())
